...
def test_wheelhouse_contains_reporter_and_verified_dependencies(platform_name, abi):
    ...
    expected_version = next(entry["reporter_version"]
        for entry in json.loads((PLUGIN_ROOT / "reporter-manifest.json").read_text())["wheelhouses"]
        if entry["platform"] == platform_name and entry["python"] == abi.removeprefix("cp"))
    assert any(name.startswith(f"shopops_reporter-{expected_version}-") for name in files)
    ...
    with ZipFile(reporter_path) as wheel:
        assert f'__version__ = "{expected_version}"' in wheel.read("shopops_reporter/__init__.py").decode()
