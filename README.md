...
`shopops-onboarding 0.1.13+codex.20260908094901` 用于安全安装、更新和诊断
`ShopOps Reporter 0.5.1`，并提供经确认的项目接入和运行结果契约配置指引。

Reporter 0.5.1 于 2026-09-08 发布，开发者安装分支为
...
当前正式离线分发为 0.5.1，包含统一接入向导和 Windows 任务计划错误码兼容修复。
...
开发者更新插件并新建 Codex 任务后，通过 `$shopops-update` 确认升级至 0.5.1。
...
`$shopops-update`，确认目标 **0.5.1** 并升级。确认旧后台身份后正常停止旧进程，再启动新版；
...
调用 `$shopops-update` 升级到当前 **0.5.1**（包含 0.4.1 修复）。插件更新本身不会升级 Reporter，也不会重启旧后台。
